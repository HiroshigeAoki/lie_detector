...
import os, sys
sys.path.append("./src")
from src.preprocess.ngram_utils import save_ngram_tfidf_diff
...
def main():
    ...
    def process_row(row):
        return row.replace("<person>", "").replace("0", "").replace(",", "").replace(".", "").replace("。", "").replace("、", "").split()

    print("start loading werewolf data")

    with open(os.path.join(data_dir, "train-mecab-parsed.txt"), 'r') as file:
        werewolf_tokens = joblib.Parallel(n_jobs=-1)(joblib.delayed(process_row)(line) for line in tqdm(file))
    print("done")
    
    print("start loading twitter data")
    with open(twitter_path, 'r') as file:
        twitter_tokens = joblib.Parallel(n_jobs=-1)(joblib.delayed(process_row)(line) for line in tqdm(file))
    print("done")
