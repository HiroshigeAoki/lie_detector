...
def main():
    ...
    all_num_utter = pd.concat(
        (
            train.loc[:, "num_utters"],
            valid.loc[:, "num_utters"],
            test.loc[:, "num_utters"],
        ),
        axis=0,
        ignore_index=True,
    )
    fig_utter = all_num_utter.hist(
        bins=80, grid=True, xlabelsize=8, ylabelsize=8
    ).set_xticks(ticks=list(range(0, 210, 10)))
    ...
    tokenizer = SentencePieceTokenizer(
        model_file="model/sentencepiece/werewolf.model", do_lower_case=True
    )
    all_utter = pd.concat(
        (
            train.loc[:, "nested_utters"],
            valid.loc[:, "nested_utters"],
    ...
    subword_count = all_utter.parallel_apply(
        count_subword, tokenizer=tokenizer
    ).explode(ignore_index=True)
    fig_sub = subword_count.hist(
        bins=150, grid=True, xlabelsize=8, ylabelsize=8
    ).set_xticks(ticks=list(range(0, 600, 50)))
