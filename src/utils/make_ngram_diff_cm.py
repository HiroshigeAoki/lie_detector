...
def get_model_data_dir_test(data: str, model: str):
    if data == "nested_day":
        ...
        if model == "bigbird":
            data_dir = (
                f"outputs/nested_day_twitter/hf_bigbird/baseline/2024-01-26_053837"
            )
        elif model == "hierbert":
            data_dir = (
                "outputs/nested_day_twitter/hf_hierbert/baseline/2024-01-25_051414"
            )
        else:
            raise ValueError("model must be bigbird or hierbert")
...
def diff_each_cm(model, data):
    ...
    tp_index = confidences[
        (confidences["true label"] == 1) & (confidences["pred label"] == 1)
    ].index
    tn_index = confidences[
        (confidences["true label"] == 0) & (confidences["pred label"] == 0)
    ].index
    fp_index = confidences[
        (confidences["true label"] == 0) & (confidences["pred label"] == 1)
    ].index
    fn_index = confidences[
        (confidences["true label"] == 1) & (confidences["pred label"] == 0)
    ].index
    ...
    results = joblib.Parallel(n_jobs=-1)(
        joblib.delayed(process_group)(
            name, rows, tokenizer_name=model, output_dir=output_dir
        )
    ...
    for i, name in enumerate(cm_name):
        ...
        for ngram in range(1, 6):
            for j, another_cm in other_cm:
                ...
                pd.DataFrame(
                    calc_diff_ngrams(target, another), columns=["ngram", "difference"]
                ).to_csv(
                    os.path.join(
                        diff_output_dir, f"{name}-{another_cm}_{ngram}_gram.csv"
                    ),
                    index=False,
                )
                normed_target = Counter(
                    dict(
                        map(
                            lambda kv: (kv[0], kv[1] / results[i]["user_num"]),
                            target.items(),
                        )
                    )
                )
                normed_another = Counter(
                    dict(
                        map(
                            lambda kv: (kv[0], kv[1] / results[j]["user_num"]),
                            another.items(),
                        )
                    )
                )
                pd.DataFrame(
                    calc_diff_ngrams(normed_target, normed_another),
                    columns=["ngram", "difference"],
                ).to_csv(
                    os.path.join(
                        normed_diff_output_dir, f"{name}-{another_cm}_{ngram}_gram.csv"
                    ),
                    index=False,
                )
        ...
        for ngram in range(1, 6):
            ...
            for another_cm in other_cm:
                if not os.path.exists(
                    os.path.join(
                        data_dir, "diff", f"{name}-{another_cm}_{ngram}_gram.csv"
                    )
                ):
                    return False
                elif not os.path.exists(
                    os.path.join(
                        data_dir, "normed_diff", f"{name}-{another_cm}_{ngram}_gram.csv"
                    )
                ):
                    return False
    return True
...
def diff_nested_day():
    ...
    for model in ["bigbird", "hierbert"]:
        ...
        for name in cm_name:
            for ngram in range(1, 6):
                raw = pd.read_csv(
                    os.path.join(output_dir_raw, "raw", f"{name}_{ngram}.csv")
                )
                raw_counter = Counter(dict(zip(raw["ngram"], raw["count"])))
                filtered = pd.read_csv(
                    os.path.join(output_dir_filtered, "raw", f"{name}_{ngram}.csv")
                )
                filtered_counter = Counter(
                    dict(zip(filtered["ngram"], filtered["count"]))
                )

                diff = calc_diff_ngrams(raw_counter, filtered_counter)
                pd.DataFrame(diff, columns=["ngram", "difference"]).to_csv(
                    os.path.join(
                        output_dir_filtered, "raw-filtered", f"{name}_{ngram}.csv"
                    ),
                    index=False,
                )
                ...
                pd.DataFrame(diff, columns=["ngram", "difference"]).to_csv(
                    os.path.join(
                        output_dir_filtered, "filtered-raw", f"{name}_{ngram}.csv"
                    ),
                    index=False,
                )
