...
sys.path.append("./src/")
from src.preprocess.ngram.utils import tokenize as mecab_tokenizer

pandarallel.initialize()

bert_tokenizer = BertJapaneseTokenizer.from_pretrained(
    "cl-tohoku/bert-base-japanese-v3", additional_special_tokens=["<person>"]
)
bigbird_tokenizer = AutoTokenizer.from_pretrained(
    "nlp-waseda/bigbird-base-japanese", additional_special_tokens=["<person>"]
)


def feature_counter(nested_df):
    nested_df.loc[:, "nested_utters"] = nested_df.loc[
        :, "nested_utters"
    ].parallel_apply(_feature_counter)
...
def _feature_counter(df: pd.DataFrame):
    ...
    for _bert_subwords, _bigbird_subwords, _morphemes in zip(
        bert_subwords, bigbird_subwords, morphemes
    ):
...
def sum_mean_std(df, col_name):
    return (
        [
            np.sum(nested_utters_df.loc[:, col_name])
            for nested_utters_df in df.loc[:, "nested_utters"]
        ],
        [
            np.mean(nested_utters_df.loc[:, col_name])
            for nested_utters_df in df.loc[:, "nested_utters"]
        ],
        [
            np.std(nested_utters_df.loc[:, col_name])
            for nested_utters_df in df.loc[:, "nested_utters"]
        ],
    )
...
def player_feature_count(df, col_names):
    for col_name in col_names:
        df[f"{col_name}_sum"], df[f"{col_name}_mean"], df[f"{col_name}_std"] = (
            sum_mean_std(df, col_name)
        )
    return df
...
def calc_stats(nested_df):
    ...
    return_dict["プレイヤー数(人狼/市民)"] = (
        f"{player_num:,}({civil_num:,}/{werewolf_num:,})"
    )
    ...
    for tokenizer in ["bert", "bigbird"]:
        return_dict[f"1プレイヤーにおける平均サブワード数({tokenizer})"] = (
            nested_df.loc[:, f"num_subwords_{tokenizer}_sum"].mean()
        )
        return_dict[f"1プレイヤーにおける最大サブワード数({tokenizer})"] = (
            nested_df.loc[:, f"num_subwords_{tokenizer}_sum"].max()
        )
        return_dict[f"1プレイヤーにおける最小サブワード数({tokenizer})"] = (
            nested_df.loc[:, f"num_subwords_{tokenizer}_sum"].min()
        )
        return_dict[f"1発話における平均サブワード数({tokenizer})"] = (
            nested_df.loc[:, f"num_subwords_{tokenizer}_sum"].sum()
            / nested_df.loc[:, "num_utters"].sum()
        )
        return_dict[f"1発話における最大サブワード数({tokenizer})"] = max(
            [
                df.loc[:, f"num_subwords_{tokenizer}"].max()
                for df in nested_df.loc[:, "nested_utters"]
            ]
        )
        return_dict[f"1発話における最小サブワード数({tokenizer})"] = min(
            [
                df.loc[:, f"num_subwords_{tokenizer}"].min()
                for df in nested_df.loc[:, "nested_utters"]
            ]
        )

    return_dict["1プレイヤーにおける平均形態素数"] = nested_df.loc[
        :, "num_morphemes_sum"
    ].mean()
    return_dict["1プレイヤーにおける最大形態素数"] = nested_df.loc[
        :, "num_morphemes_sum"
    ].max()
    return_dict["1プレイヤーにおける最小形態素数"] = nested_df.loc[
        :, "num_morphemes_sum"
    ].min()
    return_dict["1発話における平均形態素数"] = (
        nested_df.loc[:, "num_morphemes_sum"].sum()
        / nested_df.loc[:, "num_utters"].sum()
    )
    return_dict["1発話における最大形態素数"] = max(
        [df.loc[:, "num_morphemes"].max() for df in nested_df.loc[:, "nested_utters"]]
    )
    return_dict["1発話における最小形態素数"] = min(
        [df.loc[:, "num_morphemes"].min() for df in nested_df.loc[:, "nested_utters"]]
    )
...
def main():
    ...
    for name, df in dataset:
        ...
        create_histogram(
            df,
            "num_utters",
            f"{name.capitalize()} Utterances Histogram",
            f"{output_dir}/{name}_utterances_histogram.png",
        )
        for tokenizer in ["bert", "bigbird"]:
            create_histogram(
                df,
                f"num_subwords_{tokenizer}_sum",
                f"{name.capitalize()} Subwords Histogram {tokenizer}",
                f"{output_dir}/{name}_subwords_histogram_{tokenizer}.png",
            )
        create_histogram(
            df,
            "num_morphemes_sum",
            f"{name.capitalize()} Morphemes Histogram",
            f"{output_dir}/{name}_morphemes_histogram.png",
        )
    ...
    create_histogram(
        all,
        "num_utters",
        "All Utterances Histogram",
        f"{output_dir}/all_utterances_histogram.png",
    )
    ...
    for tokenizer in ["bert", "bigbird"]:
        create_histogram(
            all,
            f"num_subwords_{tokenizer}_sum",
            f"All Subwords Histogram {tokenizer}",
            f"{output_dir}/all_subwords_histogram_{tokenizer}.png",
        )
    create_histogram(
        all,
        "num_morphemes_sum",
