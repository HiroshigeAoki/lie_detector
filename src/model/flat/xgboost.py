...
from src.model.flat.xgboost import XGBClassifier, plot_importance
...
class XGBoost:
    ...
    def save_results(self, test_y, y_pred, postfix=""):
        ...
        ).to_csv(f"scores{postfix}.csv")
        cm = self.cm.compute().numpy()
        pd.DataFrame(cm).to_csv("confusiond_matrix.csv", index=False)
        accuracy = accuracy_score(test_y, y_pred)
        precision_recall_f1_support_df = pd.DataFrame(
            list(precision_recall_fscore_support(test_y, y_pred)),
        ...
        for importance_type in importance_types:
            ...
            importance_df = pd.DataFrame(
                list(importance.items()), columns=["Feature", importance_type]
            )
            importance_df.sort_values(by=importance_type, ascending=False, inplace=True)
            importance_df.to_csv(
                f"feature_importance_{importance_type}{postfix}.csv", index=False
            )
        ...
        for importance_type in importance_types:
            ...
            plot_importance(
                ...
                ax=ax,
                importance_type=importance_type,
                title=f"Feature Importance{postfix} ({importance_type})",
            )
            plt.show()
