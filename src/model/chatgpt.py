...
class ChatGPT:
    def __init__(
        ...
        model_name,
        max_tokens,
        model_outputs_dict=dict(indices=[], preds=[], labels=[], reasons=[]), #system_fingerprint=[]),
    ):  
    ...
    def on_test_end(self):
        ...
        pd.DataFrame([metrics.cpu().numpy() for metrics in test_metrics.values()], index=test_metrics.keys()).to_csv('scores.csv')
