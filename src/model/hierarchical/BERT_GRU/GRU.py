...
class GRU(nn.Module):
    ...
    def forward(self, input_ids, lengths):
        ...
        packed_data = rnn_utils.pack_padded_sequence(
            input_ids, lengths, batch_first=True, enforce_sorted=False
        )
        ...
        attention_mask = (
            torch.arange(max_len).expand(batch_size, max_len)
            >= output_lengths.unsqueeze(1)
        ).to(input_ids.device)
        attentions, weighted_outputs = self.attention(output, attention_mask)

        return dict(weighted_outputs=weighted_outputs, attentions=attentions)
