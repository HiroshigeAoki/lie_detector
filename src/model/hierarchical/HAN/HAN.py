...
from src.model.hierarchical.AttentionWithContext import AttentionWithContext
...
class HierAttnNet(AbstractHierModel):
    def __init__(
        ...
        **kwargs
        ...
        self.build_wordattennet(
            **wordattennet_params, embedding_matrix=embedding_matrix
        )
    ...
    def forward(self, nested_utters, labels, attention_mask, pad_sent_num, **kwargs):
        x = nested_utters.permute(
            1, 0, 2
        )  # X: (batch_size, doc_len, sent_len) -> x: (doc_len, bsz, sent_len)
        attention_mask = attention_mask.permute(1, 0, 2)
        attention_mask = attention_mask == 0
        word_h_n = torch.zeros(
            2, nested_utters.shape[0], self.hparams.word_hidden_dim, device=self.device
        )
        ...
        return dict(
            loss=loss,
            preds=preds,
            word_attentions=self.sent_a,
            sent_attentions=self.doc_a,
            **kwargs
        )
...
class WordAttnNet(pl.LightningModule):
    def __init__(
        ...
        self.word_atten = AttentionWithContext(
            hidden_dim * 2
        )  # since GRU is bidirectional
    ...
    def forward(self, X, h_n, attention_mask):
        ...
        if self.hparams.embed_drop:
            embed = embedded_dropout(
                self.word_embed,
                X.long(),
                dropout=self.hparams.embed_drop if self.training else 0,
            )
        else:
...
class SentAttnNet(pl.LightningModule):
    ...
    ):  # will receive a tensor of dim(bsz, doc_len, word_hidden_dim * 2)
