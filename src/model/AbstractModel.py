...
# 階層・非階層モデルに共通する処理(各ステップ、指標計算)をまとめた抽象クラス
class AbstractModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        try:
            ...
            self.log_dict(
                self.train_metrics(outputs["preds"].argmax(dim=1), batch["labels"]),
                sync_dist=True,
            )
            return outputs["loss"]
            ...
            if self.notifier:
                self.notifier.notify(
                    operation="training", status="failed", message=str(e)
                )
            raise e
    ...
    def on_train_epoch_end(self):
        ...
        if self.notifier:
            self.notifier.notify(
                operation="training", status="success", message="training finished"
            )
    ...
    def validation_step(self, batch, batch_idx):
        try:
            ...
            self.log_dict(
                self.valid_metrics(outputs["preds"].argmax(dim=1), batch["labels"]),
                sync_dist=True,
            )
            return outputs["loss"]
        except Exception as e:
            if self.notifier:
                self.notifier.notify(
                    operation="validation", status="failed", message=str(e)
                )
            raise e
    ...
    def on_validation_epoch_end(self):
        ...
        if self.notifier:
            self.notifier.notify(
                operation="validation", status="success", message="validation finished"
            )
    ...
    def on_test_epoch_end(self):
        try:
            ...
            ).to_csv(f"{self.logger.log_dir}/scores.csv")
            ...
            if self.notifier:
                self.notifier.notify(
                    operation="testing", status="success", message="testing finished"
                )

        except Exception as e:
            if self.notifier:
                self.notifier.notify(
                    operation="saving results", status="failed", message=str(e)
                )
    ...
    def on_test_end(self):
        ...
        if self.trainer.is_global_zero:  # Only on process 0
            ...
            df = pd.DataFrame(
                logits_softmax.numpy(), columns=["confidence 0", "confidence 1"]
            )
            ...
            scores_df.to_csv(
                os.path.join(self.logger.log_dir, "precision_recall_fscore_support.csv")
            )
            self.print(f"f1_precision_accuracy\n{scores_df.to_string()}")

    def configure_optimizers(self):
        return hydra.utils.instantiate(
            self.hparams.optim.args, params=self.parameters()
        )
