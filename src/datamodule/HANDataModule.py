...
class CreateHANDataset(Dataset):
    def __init__(
        ...
        df: pd.DataFrame,
        batch_size: int,
        tokenizer: HANTokenizer,
        is_murder_mystery_data: bool = False,
    ):
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        assert encoding.shape == (
            self.tokenizer.doc_length,
            self.tokenizer.sent_length,
        ), f"encoding shape: {encoding.shape} is wrong."

        item = dict(
            nested_utters=encoding,
            labels=torch.tensor(labels),
            attention_mask=attention_mask,
            pad_sent_num=pad_sent_num,
        )
        if self.is_murder_mystery_data:
            item.update(
                dict(
...
class CreateHANDataModule(pl.LightningDataModule):
    def __init__(
        self, data_dir: str, tokenizer: HANTokenizer, batch_size: int, **kwargs
    ):
        super().__init__()
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit":
            self.train_ds = CreateHANDataset(
                pd.read_pickle(os.path.join(self.data_dir, "train.pkl")),
                batch_size=self.batch_size,
                tokenizer=self.tokenizer,
                **self.kwargs,
            )
            self.valid_ds = CreateHANDataset(
                pd.read_pickle(os.path.join(self.data_dir, "valid.pkl")),
                batch_size=self.batch_size,
                tokenizer=self.tokenizer,
                **self.kwargs,
            )
        ...
        if stage == "test" or stage == "predict" or stage is None:
            self.test_ds = CreateHANDataset(
                pd.read_pickle(os.path.join(self.data_dir, "test.pkl")),
                batch_size=self.batch_size,
                tokenizer=self.tokenizer,
                **self.kwargs,
            )
