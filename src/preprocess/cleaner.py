...
def replace_term(sent: str) -> str:
    names = [
        ...
        "青年 ヨアヒム|ヨア(ヒム)*|ヨアヒ|よあひー|よあひむ|青年",
        "少女 リーザ|リ(ーズ|ーザ|ザ)+|少女",
        "宿屋の女主人 レジーナ|おばさん|マダム|レジ(ーナ)*|レジナ|姐さん|姐御|宿屋の女主人|宿屋のおかみ|女将|小母様|レジーナ",
    ]
    name_char = "楽|村|長|老|爺|神|木|樵|旅|者|年|尼|少|妙|女|好|行|商|羊|屋|青|娘|農|宿|姐|修|尼|仕|服|司|書|兵|傷|負"
    nick_names = '|'.join([
        "エル",
        "ルナ",
        "オト",
    ...
    particles = '|'.join(["は","も","を","が","か","で","と","に","の","や","へ"])
    honorific_titles = '|'.join([
        "姉", "ね[え|ぇ]", "お姉", "おね[え|ぇ]", "お嬢",
        "兄", "に[い|ぃ]", "お兄", "おに[い|ぃ]",
        "おじいちゃん", "爺", "翁", "じ[い|ぃ]",
        "さん|サン","くん|クン|君", "ちゃん|チャン", "坊|ぼう", "子",
        "様", "さま", "殿", "どの",
        "どん", "さぁ", "はん",
    ...
    freq_subsequent_words = "どっち|的には|のように|co|判定|発言|内心|視点|確定|確白|確黒|希望|両狼|確認|考察|襲撃|護衛|直吊|吊|即吊|対抗|ライン|連打|不在|パンダ|ぱんだ|墓|ごめん|ありがとう|食[っ|べ|う]"
    ...
    reg2 = regex.compile(rf"(?V1)(?<!({ignore_prefix}))({name_char}|{nick_names})(?=(({particles}|{symbols})*({freq_subsequent_words}|{role_names}|{name_char}|{nick_names})|{symbols}|<person>|{particles}|{honorific_titles}))")
