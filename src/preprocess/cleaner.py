...
def replace_term(sent: str) -> str:
    ...
    name_char = '|'.join([
        ...
        "(?<!(この|漁|山|寒|農))村(?!(起こし|を上げて|落|八分|雨))",
        "(?<!(身|発話|発言|館|機|店|校|学科|委員|総|番|首|家|冗|助|成|百薬の))長(?!(い|短|兄|姉|身|所|いも|芋|編|蛇|閑))",
        "(?<!(不|養))老(?!(獪|い|け|化|害|眼|若男女))",
        "(?<!)爺(?!)",
        "(?<!(ゼウス|ぜうす|太陽|恵みの|鬼|疫病|禍|氏|女))神(?!(社|宮|様|のみぞ|聖|に祈る|速|罰|秘|童|出鬼没|経|[無|在]月))",
        ...
        "(?<!(昨|去|来|今|往|長|永|各|享|凶|更|周|若|熟|積|中|定|晩|忘|没|歴|暦|留|豊|厄|[一二三四五六七八九十百千万億兆京]))年(?!(内|間|始|初|末|明|度|頭|譜|頃|寄|賀|端|功|中|来|齢|次|貢|金|号|輩|配|輪|をとった))",
        "(?<!(極|希|減|稀|最|些|過|多|幼|少))少(?!(々|し|ない|数|額|量|数))",
        "(?<!(絶|微|奇|巧|軽|珍|言い得て))妙(?!(に|な|薬|策|妙|計|齢|手|味|法))",
        "(?<!(男|悪|下|乙|鬼|次|海|山|売|彼))女(?!(性|の子|権|系|王))",
        "(?<!(同|嗜|恰|愛|絶|選り|えり|大|物|良|格))好(?!(意|感|み|悪|機|奇|例|物|調|適|転|敵手|日|事||評|む|ま|き))",
        "(?<!(同|続|鈍|徐|退|進|運|逆|血|徐|随|両|並|平|飛|夜|連|紀|危|銀|品|洋|所|諸|移|横|愚|蛇|凶|飛|非|動|苦|急|強|慣|刊|奉|奥|横|奇|逆|現|孝|興|試|施|執|巡|代|単|知|通|発|尾|犯|蛮|道|徳|断|昼|草|素|千里の|遂|心|巡|順|実))行(?!(動|政|い|き先|書|間|列|儀|幸|事|為|軍|程|楽|使|為|文|雲|跡|政|状|水|脚|火))",
    ...
    ignore_prefix = "進|続|奇|この|確定|うちの|勝|色々|流|隣"
    ignore_suffix = "い|う|な|し|性|っ|[き-け]|騙り|の開始|に泊|方ない|ぃ*切って"
    freq_subsequent_words = "どっち|的には|のように|co|判定|発言|内心|視点|確定|確白|確黒|希望|両狼|確認|考察|襲撃|護衛|直吊|吊|即吊|対抗|ライン|連打|不在|パンダ|ぱんだ|ステルス|墓|ごめん|ありがとう|食[っ|べ|う]"
