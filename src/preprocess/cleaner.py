...
# デバッグ用に置換された部分を控えておく。
def add_to_replacement_track_dict(
    sent: str,
    pattern_name: str,
    matched_text: str,
    replacement: str,
    replacement_track_dict: dict,
    ex_num: int = 3,
):
    ...
    if len(replacement_track_dict[matched_text]["examples"]) < ex_num:
        replacement_track_dict[matched_text]["examples"].add(sent)


def track_replacements(
    sent, pattern_name, pattern, replacement, replacement_track_dict
):
    def replace_func(match):
        matched_text = match.group(0)
        add_to_replacement_track_dict(
            sent=sent,
            pattern_name=pattern_name,
            matched_text=matched_text,
            replacement=replacement,
            replacement_track_dict=replacement_track_dict,
        )
        return replacement
    ...
    if (
        pattern_name in ["remove_terms_with_pipe", "remove_with_char_first_letter"]
        and remained_ratio < 0.8
    ):
        add_to_replacement_track_dict(
            sent=sent,
...
def clean_sent(sent: str, replacement_track_dict: dict) -> str:
    ...
    patterns_replacements = [
        ...
        (
            "remove_kaomoji_left",
            left_right_wing + left_cheek + mouse + left_right_wing,
            "",
        ),  # 左頬あり
        (
            "remove_kaomoji_right",
            left_right_wing + mouse + right_cheek + left_right_wing,
            "",
        ),  # 右頬あり
        ("remove_.o0", r"\.+o0", ""),
        ("remove_whitespace", r"[\t\n\r]", ""),
...
def replace_term(sent: str, replacement_track_dict: dict) -> str:
    ...
    first_letters = (
        r"ア|ヴ|エ|オ|カ|ク|ゲ|ジ|シ|デ|ディ|ト|ニ|ス|パ|フ|ペ|モ|ヤ|ヨ|リ|レ"
    )
    ...
    patterns_replacements = [
        ...
        (
            "game_strategies",
            rf"(が[{color}]|は[{color}]|[{color}]っぽい|[{color}]寄り|[{color}]と思う|[{color}]だと思う|[{color}]でいい|[{color}]決め打ち|[{color}]で見|[{connections}][{color}]|[{color}]確定?|が偽|偽っぽい|白判定|白確定?|白く|は白|黒判定|黒確定?|黒い|黒く|グレースケール|グレーゾーン|グレー|能力者|役職者|人外|人狼|人間|突然死|仮決定)",
            "",
        ),
        (
            "game_actions",
            r"(寡黙|狼が|狼を|狼の|が狼|占い先|占い対象|占われた|占わ|吊っちゃった|吊った|占っ|占う|共吊り|吊り|吊る|吊って|吊られそう|吊られる|吊ろうとした|吊ろう|吊れ|吊ら|co|護衛|占い|襲撃|潜伏|対抗|狩り|狩って|狩る|襲い|襲う|襲った|ローラー|ステルス|ロラ|スライド|発表順了解|発表順も了解)",
            "",
        ),
        (
            "general_game_terms",
            r"(占い希望|ライン|灰考察|本決定|議事録|仮決定|墓下|投票|fo|log|gj|gs|sg|rp)",
            "",
        ),
        ...
        (
            "preserve",
            r"旅(?=(の|に|の途中|の途中|に出る|に出る|に|が好き|好き))",
            "@旅@",
        ),
        ...
        (
            "preserve",
            rf"村(?=(のために|として|の総意|[はの]負け|を滅ぼ|[は|の]勝|へ|々|{particles}))",
            "@村@",
        ),
        ...
        (
            "remove_char_with_connections",
            f"({name_char}|{first_letters}|{short_role_names}|{short_term})+({connections}|0){1,}({name_char}|{first_letters}|{short_role_names}|{short_term}|{connections}|0|<person>)*",
            "",
        ),
        (
            "remove_name_person_char1",
            rf"(?<=({person_char_pattern}|{short_char_terms}))({name_char})+",
            "",
        ),
        (
            "remove_name_person_char2",
            rf"({name_char})+(?=({person_char_pattern}|{short_char_terms}))",
            "",
        ),
        (
            "remove_name_person_kana1",
            rf"(?<=({name_char}|<person>|{short_char_terms}))({first_letters})+",
            "",
        ),
        (
            "remove_name_person_kana2",
            rf"({first_letters})+(?=({name_char}|<person>|{short_char_terms}))",
            "",
        ),
        (
            "replace_name",
            rf"({nick_names}|{name_char})(?=([{particles}|{symbols}|{freq_words}|{short_char_terms}|{nick_names}|{honorific_titles}]))",
            "<person>",
        ),
        ("preserve", "@旅@行@", "旅行"),
        ("preserve", "@長@旅@", "長旅"),
        ...
        ("preserve", "@女@好@き@", "女好き"),
        (
            "reduce_repeated_patterns",
            rf"(<person>|<person>0|<person>>|<person>・)\1+",
            r"\1",
        ),
        ("preserve", "言霊", "@言@霊@"),
        ("preserve", "全然", "@全@然@"),
        (
            "remove_char2",
            rf"(?:は|が)(?:{short_char_terms})|(?:{short_char_terms})(?:ならば|なら|だ|かも|確定|確|が確定)",
            "",
        ),
        (
            "remove_term_char1",
            rf"(?<=({person_char_pattern}|{short_char_terms}))({short_char_terms})+",
            "",
        ),
        (
            "remove_term_char2",
            rf"({short_char_terms})+(?=({person_char_pattern}|{short_char_terms}))",
            "",
        ),
        ("preserve", "@言@霊@", "言霊"),
        ("preserve", "@全@然@", "全然"),
...
def clean_murder_mystery_data(sent, replacement_track_dict):
    patterns_replacements = [
        (
            "見やるは、誰ぞ、",
            r"(八木|ヤギ|マトン|夜見|ヨミ|オコジョ|仁江|ニモ|モズ|持衰|ジサイ|サシミ)",
            "<person>",
        ),
        (
            "ワタナベ1",
            r"(南極の(渡辺|わたなべ|ワタナベ)|沖縄の(渡部|わたなべ|ワタナベ)|スウェーデンの(渡邉|わたなべ|ワタナベ))",
            "<person>",
        ),
        ...
        (
            "死者の湯加減",
            r"(ダイヴ侍|ダイヴ|ダイブ|侍|小泉潤|小泉|潤|花咲ラン|花咲|ラン|記本文章|記本|基本|赤木巧|赤木|巧|小泉充之介|充之介)",
            "<person>",
        ),
        ("mention", r"<@\w+>", "<person>"),
        ("mention utter", r"<>", ""),
    ...
    for args in patterns_replacements:
        sent = track_replacements(
            sent, *args, replacement_track_dict=replacement_track_dict
        )
