...
class DataProcessor:
    def __init__(
        ...
        self.excluded_utters_logger.addHandler(
            logging.FileHandler(os.path.join(save_dir, "excluded_utters.txt"), mode="w")
        )
    ...
    def save_to_pickle(self, raw_nested_utters, labels, users, mode):
        try:
            ...
            df = pd.DataFrame(
                {
                    "nested_utters": raw_nested_utters,
            ...
            self.notifier.notify(
                operation=f"save_to_pickle({mode})",
                status="failed",
                message=str(traceback.format_exc()),
            )
            raise

    def exclude_werewolf_specific_utterances(
        ...
        try:
            ...
            self.logger.info(
                f"- {mode} - Splitting {total_rows} rows of DataFrame into {num_batches} batches "
            )

            df_split = np.array_split(df, num_batches)

            assert all(
                isinstance(batch, pd.DataFrame) for batch in df_split
            ), "df_split contains non-DataFrame elements."
            assert all(
                not batch.empty for batch in df_split
            ), "df_split contains empty DataFrame."

            results = joblib.Parallel(n_jobs=self.batch_num)(
                joblib.delayed(self.process_batch)(index, batch, mode)
                for index, batch in enumerate(df_split)
            )
            ...
            for col in expected_columns:
                ...
                ), f"Final DataFrame is missing expected column {col}"
    ...
    def process_batch(self, index, batch_df, mode):
        try:
            ...
            self.logger.info(
                f"- {mode} - Starting to process batch {index} with {batch_size} rows"
            )

            try:
                # Initialize tokenizer and model for each batch here
                self.logger.info(
                    f"- {mode} - Loading BBS tokenizer and model for batch {index}"
                )
                BBStokenizer = CamembertTokenizer.from_pretrained(
                    self.BBS_tokenizer_path
                )
                BBSmodel = RobertaForSequenceClassification.from_pretrained(
                    self.BBS_model_path
                )
                BBSmodel.to(self.device)
                self.logger.info(
                    f"- {mode} - Loaded BBS tokenizer and model for batch {index}"
                )
            except Exception as e:
            ...
            for row in batch_df.itertuples():
                try:
                    ...
                    nested_utters = self._exclude_werewolf_specific_utterances(
                        nested_utters["raw_nested_utters"].tolist(),
                        BBStokenizer,
                        BBSmodel,
                    )
                    batch_df.at[row.Index, "nested_utters"] = pd.DataFrame(
                        nested_utters, columns=["raw_nested_utters"]
                    )
                    self.logger.info(
                        f"- {mode} - Processed row {row.Index + 1} in batch {index}"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to process row {row.Index + 1} in batch {index}: {e}"
                    )
            ...
            self.notifier.notify(
                operation=f"batch {index} processing in {mode}",
                status="failed",
                message=str(e),
            )
            raise

    def _exclude_werewolf_specific_utterances(
        self, nested_utterances: list, BBStokenizer, BBSmodel
    ):
    ...
    def run(self, data_dir="data/nested"):
        ...
        try:
            for mode in ["train", "valid", "test"]:
                ...
                self.logger.debug(
                    f"{df['nested_utters'].apply(lambda x: len(x)).describe()}"
                )
            ...
            self.notifier.notify(
                operation="extract_bbs.py", status="failed", message=str(e)
            )
            return
...
if __name__ == "__main__":
    ...
    BBS_tokenizer_path = (
        "/home/haoki/Documents/vscode-workplaces/sotuken/tokenizing/tokenizer/"
    )
