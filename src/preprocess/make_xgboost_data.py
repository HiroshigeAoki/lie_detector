...
def create_ngram_features(nested_utters, ngram_dict):
    ...
    for n_gram in range(1, 6):
        ...
        normed_counts = {key: ngrams_list.count(key) / total_ngram for key in ngram_dict[f"{n_gram}_gram"]}
        ngram_features.update(normed_counts)
    return ngram_features


def create_ngram_dict(count_dir, n_gram, top_n):
    ngram_label_0 = pd.read_csv(f"{count_dir}/ngram/normed_diff/{n_gram}_gram_count_0.csv")["ngram"][:top_n].tolist()
    ngram_label_1 = pd.read_csv(f"{count_dir}/ngram/normed_diff/{n_gram}_gram_count_1.csv")["ngram"][:top_n].tolist()
...
def create_ngram_features_for_df(df, ngram_dict):
    ngram_features = pd.DataFrame(df["nested_utters"].parallel_apply(create_ngram_features, args=(ngram_dict,)).tolist())
...
def main(args):
    ...
    if os.path.exists(os.path.join(data_dir, "train.pkl")):
        train = pd.read_pickle(os.path.join(data_dir, "train.pkl")).drop(["users"], axis=1)
        datasets.append((train, "train"))
    if os.path.exists(os.path.join(data_dir, "valid.pkl")):
        valid = pd.read_pickle(os.path.join(data_dir, "valid.pkl")).drop(["users"], axis=1)
        datasets.append((valid, "valid"))
    if os.path.exists(os.path.join(data_dir, "test.pkl")):
        test = pd.read_pickle(os.path.join(data_dir, "test.pkl")).drop(["users"], axis=1)
        datasets.append((test, "test"))
    
    print("creating ngram dict...")
    ngram_dicts = joblib.Parallel(n_jobs=5)(joblib.delayed(create_ngram_dict)(count_dir, n_gram, top_n) for n_gram in range(1, 6))
    ngram_dict = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default="nested_day/parsed", help="XGBoost用のデータを作りたいデータ(parsedを作らなくてもいい)")
    parser.add_argument("--count", type=str, default="nested_day/under_sampling_160000", help="ngramのカウントデータがあるディレクトリ。このカウントの内、各n_gramの各ラベル毎に上位top_nを特徴量として使う。なので、学習データとテストデータのcountディレクトリは同じものを使う必要がある。")
