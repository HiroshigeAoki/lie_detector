...
def generate_ngrams_from_file(file, n_gram):
    ...
    for line in lines:
        ngrams.extend(
            generate_ngrams_for_sentence(
                [neologdn.normalize(token.lower()) for token in line.split()],
                n_gram=n_gram,
            )
        )
    return ngrams
...
def main(args):
    ...
    n_gram_counter = Counter(
        list(
            itertools.chain(
                *joblib.Parallel(n_jobs=len(files))(
                    joblib.delayed(generate_ngrams_from_file)(file, n_gram)
