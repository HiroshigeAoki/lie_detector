...
def process_ngram(n_gram, data_dir, output_dir, label_0_user_num, label_1_user_num):
    ...
    with open(os.path.join(output_dir, f"{n_gram}_gram_1.pkl"), "rb") as f:
        ...
        pd.DataFrame(label_0_vocab_count.most_common(), columns=['ngram', 'count']).to_csv(
            os.path.join(output_dir, "raw", f"{n_gram}_gram_count_0.csv"), index=False)
        pd.DataFrame(label_1_vocab_count.most_common(), columns=['ngram', 'count']).to_csv(
        ...
        normed_0_count = Counter(dict(map(lambda kv: (kv[0], kv[1] / label_0_user_num), label_0_vocab_count.items())))
        normed_1_count = Counter(dict(map(lambda kv: (kv[0], kv[1] / label_1_user_num), label_1_vocab_count.items())))
        pd.DataFrame(calc_diff_ngrams(normed_0_count, normed_1_count), columns=['ngram', 'difference']).to_csv(
            os.path.join(output_dir, "normed_diff", f"{n_gram}_gram_count_0.csv"), index=False)
        pd.DataFrame(calc_diff_ngrams(normed_1_count, normed_0_count), columns=['ngram', 'difference']).to_csv(
            os.path.join(output_dir, "normed_diff", f"{n_gram}_gram_count_1.csv"), index=False)
        
        # 正規化無し
        os.makedirs(os.path.join(output_dir, "diff"), exist_ok=True)
        pd.DataFrame(calc_diff_ngrams(label_0_vocab_count, label_1_vocab_count), columns=['ngram', 'difference']).to_csv(
            os.path.join(output_dir, "diff", f"{n_gram}_gram_count_0.csv"), index=False)
        pd.DataFrame(calc_diff_ngrams(label_1_vocab_count, label_0_vocab_count), columns=['ngram', 'difference']).to_csv(
...
def main(args):
    ...
    parsed_0 = pd.read_pickle(os.path.join(data_dir, f"label_0_tokens_{args.tokenizer_name}.pkl"))
    parsed_1 = pd.read_pickle(os.path.join(data_dir, f"label_1_tokens_{args.tokenizer_name}.pkl"))
    
    label_0_user_num = len(parsed_0)
    label_1_user_num = len(parsed_1)
    
    joblib.Parallel(n_jobs=5)(joblib.delayed(process_ngram)(n_gram, data_dir, output_dir, label_0_user_num, label_1_user_num) for n_gram in range(1, 6))
