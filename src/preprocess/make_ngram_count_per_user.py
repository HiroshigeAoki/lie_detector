...
def process_all_groups(groups):
    return Parallel(n_jobs=-1)(delayed(process_user_group)(group) for group in tqdm(groups, desc="Processing groups"))
...
def main(args):
    ...
    exclude_tokens = ["＜", "＞", "person"]#, '＞_＜', '＞_', '_＜', 'ジムソン', '＞_※＜', '⑥', 'ーダ', 'kwsk', '?＞＞＜', '片片', '服そ', 'ゃら', 'ゲス', '※', 'ワクテカ', 'ーー',
    ...
    label_0_tokens_list = tokenize_group(
        label_0_utters_per_user, exclude_tokens=exclude_tokens, full_width_person_token=True, tokenizer_name=args.tokenizer_name)
    
    
    print("Tokenizing label 1 data...")
    label_1_tokens_list = tokenize_group(
        label_1_utters_per_user, exclude_tokens=exclude_tokens, full_width_person_token=True, tokenizer_name=args.tokenizer_name)
    
    # Saving token data
    pd.DataFrame({"users": label_0_user_names, "tokens": label_0_tokens_list}).to_pickle(os.path.join(data_dir, f"label_0_tokens_{args.tokenizer_name}.pkl"))
    pd.DataFrame({"users": label_1_user_names, "tokens": label_1_tokens_list}).to_pickle(os.path.join(data_dir, f"label_1_tokens_{args.tokenizer_name}.pkl"))
    
    for n_gram in [1, 2, 3, 4, 5]:
        print(f"Processing {n_gram}-gram for label 0...")
        label_0_ngrams_counters = generate_ngrams_counter_per_user(label_0_tokens_list, label_0_user_names, n_gram)
        
        print(f"Processing {n_gram}-gram for label 1...")
        label_1_ngrams_counters = generate_ngrams_counter_per_user(label_1_tokens_list, label_1_user_names, n_gram)
        
        for i, counts in enumerate(tqdm([label_0_ngrams_counters, label_1_ngrams_counters], desc=f"Saving {n_gram}-gram data")):
