...
sys.path.append("./src")
from src.preprocess.ngram.utils import tokenize
...
def tokenize_nested_utters(nested_utters, token_set):
    ...
    df = pd.DataFrame(
        dict(
            raw_nested_utters=nested_utters["raw_nested_utters"]
            .apply(
                tokenize,
                token_set=token_set,
                full_width_person_token=False,
...
def main():
    ...
    for df, mode in data:
        results = df["nested_utters"].parallel_apply(
            lambda x: tokenize_nested_utters(x, token_set)
        )
    ...
    with open(
        f"data/nested_day_twitter/excluded_tokens_counter.pkl", "wb"
    ) as pickle_file:
    ...
    print(
        "\n".join(
            list(
                map(lambda x: f"{x[0]}: {x[1]}", excluded_tokens_counter.most_common())
            )
        ),
        file=open("data/nested_day_twitter/excluded_tokens_count.txt", "w"),
    )
