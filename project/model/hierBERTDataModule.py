...
class CreateHierBertDataModule(pl.LightningDataModule):
    def __init__(self, data_dir, max_len_sent, max_len_doc, batch_size=16):
        super().__init__()
        ...
        self.batch_size = batch_size
        self.tokenizer = hierBertTokenizer(max_len_sent=max_len_sent, max_len_doc=max_len_doc)
        self.n_cpus = os.cpu_count()

    def setup(self, stage: Optional[str] = None):
        # set train and valid dataset
        if stage == 'fit':
            self.train_ds = CreateDataset(self.train_df, self.batch_size, self.tokenizer)
            self.valid_ds = CreateDataset(self.valid_df, self.batch_size, self.tokenizer)
        # set test dataset
        if stage == 'test' or stage is None:
            self.test_ds = CreateDataset(self.test_df, self.batch_size, self.tokenizer)
